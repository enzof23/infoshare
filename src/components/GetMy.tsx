...
export default function () {
  const words = ["resume", "portfolio", "github", "contacts"];
  ...
  return (
    <div className="w-[107px] xs:w-[130px] sm:w-[194px] lg:w-[258px] text-center word-slider-container">
      {words.map((word, index) => (
        <span
          key={index}
          className={`word ${index === currentIndex ? "active" : ""} ${
            index === nextIndex ? "next" : ""
          } xs:w-full xs:text-center font-extrabold uppercase bg-accent-gradient bg-size-500 bg-clip-text text-transparent underline decoration-dashed decoration-accent decoration-[3px] underline-offset-8`}
        >
          {word === "github" ? (
            <div className="flex justify-center items-center gap-1 sm:gap-2">
              {word}
              <svg
                className="w-7 h-7 xs:w-8 ws:h-8 sm:h-10 sm:w-10 lg:h-12 lg:w-12 fill-accent-secondary"
                role="img"
                viewBox="0 0 24 24"
            ...
            <div className="flex justify-center items-center gap-1 sm:gap-2">
              {word}
              <svg
                className="w-6 h-6 xs:w-8 ws:h-8 sm:h-10 sm:w-10 lg:h-12 lg:w-12 fill-accent-secondary"
                role="img"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
